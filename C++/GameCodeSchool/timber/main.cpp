...
// Where is the player/branch? Possible locations
...
int main()
{
	// Create a video mode object from the class VideoMode
    ...
    //give sprites the new tree texture
	Sprite spriteTree2;
	...
	// variable that controls how fast the bee zooms across the screen
	float beeSpeed = 0.0f;
	...
	// Variables to control in game timer
	Clock clock;
    ...
    //create text objects to be displayed later
	Text messageText;
	Text scoreText;
	Text fpsText;
	...
	// apply our font to the texts we created earlier
	...
	messageText.setString("'Enter' to Start! Arrow Keys to Play!");
	...
	// create background for the text using same tool for the time bar
	...
	while (window.isOpen())
	{
		//listen for when key is unpressed;
		Event event;
		...
		*/
		...
		if (acceptInput)
		{
			...
			// actions when RIGHT Arrow Key is Pressed
			if (Keyboard::isKeyPressed(Keyboard::Right))
			{
				// Set the player on the right
		...
		*/
		...
		// Draw the 3 clouds using a for loop, old code below
		...
		// draw trees with enhanced texture/graphic
